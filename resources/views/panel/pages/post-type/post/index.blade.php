    ...
    <div class="container-fluid py-3">
        <div class="d-flex align-items-center justify-content-between">
            ...
            <div>
                ...
                <a class="btn btn-outline-secondary" href="{{ route('panel.post-types.categories.index', ['post_type'=> $post_type]) }}">Kategori</a>
                @endif
                <a class="btn btn-primary" href="{{ route('panel.post-types.posts.create', ['post_type'=> $post_type]) }}">Tambah</a>
            </div>
        </div>
    </div>
@endsection
...
$(function(){
    const dt = $('#dt').DataTable({
        ...
        ajax : {
            url : `{{ route('panel.post-types.posts.index', ['post_type'=> $post_type]) }}`
        },
        columns : [
            { data: 'title' },
            { data: 'published_at' },
            { 
                ...
                render(data, type, row, meta){
                    ...
                    let className = condition ? 'success' : 'secondary';
                    let text = condition ? 'Terbit' : 'Draf'
                ...
                render(data){
                    return `<div class="dropdown dropstart">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end border shadow">
                            <li><a class="dropdown-item" href="{{ route('panel.post-types.posts.index', ['post_type'=> $post_type], ['post_type'=> $post_type]) }}/${data}/edit">Edit</a></li>
        ...
        rowCallback(row, data) {
            ...
            $(row).find('.btn-delete').click(function(){
                let primaryKey = $(this).data('primaryKey'),
                    url = `{{ route('panel.post-types.posts.index', ['post_type'=> $post_type]) }}/${primaryKey}`;
